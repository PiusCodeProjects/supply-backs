...
/**
 * Single source of truth for the contractor's conversation list.
 *
 * Why this exists:
...
const CACHE_KEY = 'cscp_conversations_v1';
...
export function MessagingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Real-time list updates. A new message bumps its conversation to the top,
  // refreshes the preview, and increments unread (unless we sent it).
  useEffect(() => {
    if (!socket) return;
    const onMessageCreated = (payload: any) => {
      setConversations(prev => {
        ...
        if (idx < 0) {
          // First message in a brand-new conversation we don't know about yet.
          refresh();
          return prev;
        }
        const c = prev[idx];
    ...
    const onConversationRead = (payload: any) => {
      // Only our own "I read this" events should clear the badge.
