...
export async function apiRequest<T = unknown>(
  ...
  try {
    ...
    throw new Error('API server is unavailable. Make sure the backend is running.');
  }
...
export const projectsApi = {
  setRequirements: (token: string, projectId: string, requirements: any[]) =>
    apiRequest(`/projects/${projectId}/requirements`, {
      method: 'POST',
      token,
      body: { requirements },
    }),
