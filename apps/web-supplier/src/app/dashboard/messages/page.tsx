...
function avatarColor(str: string) {
  const p = ['#10b981','#3b82f6','#f59e0b','#ef4444','#8b5cf6','#ec4899','#06b6d4'];
  let h = 0;
...
export default function SupplierMessagesPage() {
  ...
  const { user: currentUser, businessName } = useCurrentUser();
  ...
  // updates are all owned by MessagingContext.
  const {
    conversations,
    loading,
  ...
  // WhatsApp. Sidebar list / unread badge / room joins are handled by
  // MessagingContext.
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedGroupName && conversations.length > 0) {
      ...
      // Room joins are owned by MessagingContext.
    }
  ...
  async function handleSend() {
    ...
    // Optimistic bubble — drops the perceived send latency to ~0. The real
    // server payload (via WS or HTTP response) replaces this in place.
    const optimistic = {
      id: tempId,
      pending: true,
      content,
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .cmc-search input:focus { border-color: var(--accent-glow); background: var(--border-subtle); }
        ...
        .cmc-item.active { background: var(--accent-muted); }
        ...
        .cmc-progress-fill { height: 100%; background: linear-gradient(90deg, transparent, var(--accent-glow), transparent); transition: none; }
        ...
        .cmc-bubble.own .cmc-reply-preview { border-left-color: rgba(255,255,255,0.4); }
        ...
        .cmc-bubble.own .cmc-time { color: rgba(255,255,255,0.75); }
        ...
        .cmc-bubble.own .cmc-file-link { color: rgba(255,255,255,0.9); }
        ...
        .cmc-fab:hover { background: var(--accent); color: #fff; border-color: var(--accent); transform: scale(1.08); }
        ...
        .cmc-reply-bar {
          ...
          background: var(--accent-muted); border-radius: 0 8px 8px 0;
          margin-bottom: 10px;
        }
        ...
        .cmc-composer:focus-within { border-color: var(--border-accent); box-shadow: 0 0 0 3px var(--accent-muted); }
        ...
        .cmc-send.ready { background: var(--accent); color:#fff; box-shadow: 0 4px 14px var(--accent-glow); }
        ...
        .cmc-ctx-item { display:flex; align-items:center; gap:9px; padding:8px 13px; border-radius:7px; cursor:pointer; font-size:13px; color: var(--text-secondary); transition:background 0.1s; }
